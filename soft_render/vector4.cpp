...
Vector4D Vector4D::lerp(const Vector4D& vec, float weight) const {
	return Vector4D(
		x * (1.0f - weight) + vec.x * weight,
		y * (1.0f - weight) + vec.y * weight,
		z * (1.0f - weight) + vec.z * weight,
		w * (1.0f - weight) + vec.w * weight
	);
}
Vector4D Vector4D::lerp(const Vector4D& vec1, const Vector4D& vec2, float weight) const {
	return Vector4D(
		vec1.x * (1.0f - weight) + vec2.x * weight,
		vec1.y * (1.0f - weight) + vec2.y * weight,
		vec1.z * (1.0f - weight) + vec2.z * weight,
		vec1.w * (1.0f - weight) + vec2.w * weight
	);
}